...
def test_dispatch_only_notifies_matching_active_subscriptions(session: Session, make_user) -> None:
    ...
    matching = WebhookSubscription(
        url="https://a.example.com", secret="s1",
        subscribed_events=["ticket.status_changed"], created_by=admin.id,
    )
    wrong_event = WebhookSubscription(
        url="https://b.example.com", secret="s2",
        subscribed_events=["ticket.comment_created"], created_by=admin.id,
    )
    inactive = WebhookSubscription(
        url="https://c.example.com", secret="s3", is_active=False,
        subscribed_events=["ticket.status_changed"], created_by=admin.id,
    )
    session.add_all([matching, wrong_event, inactive])
    session.commit()

    with patch("app.services.notification_service.send_webhook_notification.delay") as mock_delay:
        dispatch_event(session, WebhookEventType.TICKET_STATUS_CHANGED, {"foo": "bar"}, ticket_id=1)
...
def test_ticket_status_change_triggers_dispatch(
    ...
    ticket = client.post(
        "/tickets", json={"title": "T", "description": "D", "category_id": category.id}, headers=headers_solic
    ).json()
...
def test_public_comment_triggers_dispatch(
    ...
    with patch("app.services.comment_service.dispatch_event") as mock_dispatch:
        client.post(
            f"/tickets/{ticket['id']}/comments", json={"content": "Atualização"}, headers=headers
        )
